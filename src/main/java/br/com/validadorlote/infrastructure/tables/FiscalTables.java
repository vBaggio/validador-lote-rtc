    ...
    static FiscalTables load(InputStream in) {
        ...
        for (JsonNode cstNode : root) {
            ...
            CstEntry entry = new CstEntry(cst, requiredText(cstNode, "nome", cstContext),
                    ...
                    requiredBoolean(cstNode, "exigeMonofasia", cstContext),
                    requiredBoolean(cstNode, "exigeReducaoBaseCalculo", cstContext),
                    requiredBoolean(cstNode, "exigeTransferenciaCredito", cstContext),
                    requiredBoolean(cstNode, "exigeCreditoPresumidoIbsZfm", cstContext),
                    requiredBoolean(cstNode, "exigeAjusteCompetencia", cstContext),
            ...
            for (JsonNode classificationNode : requiredArray(cstNode, "classificacoes", cstContext)) {
                ...
                ClassTribEntry classification = new ClassTribEntry(code,
                        ...
                        requiredBoolean(classificationNode, "exigeTributacaoRegular",
                                classificationContext),
                        requiredBoolean(classificationNode, "permiteCreditoPresumido",
                                classificationContext),
                        requiredBoolean(classificationNode, "exigeEstornoCredito",
                                classificationContext),
                        requiredBoolean(classificationNode, "exigeMonoValor", classificationContext),
                        requiredBoolean(classificationNode, "exigeMonoRetencao",
                                classificationContext),
                        requiredBoolean(classificationNode, "exigeMonoRetido", classificationContext),
                        requiredBoolean(classificationNode, "exigeMonoDiferimento",
                                classificationContext),
                        requiredBoolean(classificationNode, "exigePbioDiferenca",
                                classificationContext),
        ...
        try {
            ...
            csts.values().stream().sorted(java.util.Comparator.comparing(CstEntry::cst))
                    .forEach(entry -> digest(digest, entry.cst(), entry.nome(),
                            ...
                            Boolean.toString(entry.exigeDiferimento()),
                            Boolean.toString(entry.exigeMonofasia()),
                            Boolean.toString(entry.exigeReducaoBaseCalculo()),
                            Boolean.toString(entry.exigeTransferenciaCredito()),
                            Boolean.toString(entry.exigeCreditoPresumidoIbsZfm()),
                            Boolean.toString(entry.exigeAjusteCompetencia()), date(entry.iniVig()),
                            date(entry.fimVig())));
            classificacoes.values().stream()
                    ...
                    .forEach(entry -> digest(digest, entry.codigo(), entry.nome(), entry.cst(),
                            ...
                            Boolean.toString(entry.exigeTributacaoRegular()),
                            Boolean.toString(entry.permiteCreditoPresumido()),
                            Boolean.toString(entry.exigeEstornoCredito()),
                            Boolean.toString(entry.exigeMonoValor()),
                            Boolean.toString(entry.exigeMonoRetencao()),
                            Boolean.toString(entry.exigeMonoRetido()),
                            Boolean.toString(entry.exigeMonoDiferimento()),
                            Boolean.toString(entry.exigePbioDiferenca()),
