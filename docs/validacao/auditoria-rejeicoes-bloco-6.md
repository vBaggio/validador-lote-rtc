...
1. **1115, Exceção 2:** a tabela de combustíveis monofásicos não está embarcada. Todo item com
   `cProdANP` e sem `IBSCBS` fica `NOT_EVALUATED`, inclusive quando o código talvez não pertença à
   tabela. É falso negativo declarado, não conformidade.
